...
// Po overení, že je užívateľ prihlásený
$user_id = $_SESSION['user_id'];

// Načítame meno a priezvisko
$stmt = $conn->prepare("SELECT name, lastname FROM users WHERE id = ?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$stmt->bind_result($meno, $priezvisko);
$stmt->fetch();
$stmt->close();
    ...
    <div>
        <header>
            ...
            <div class="pravy">
                <ul>
                    <?php if(!empty($user)) { ?>
                        <li><?php echo htmlspecialchars($meno . ' ' . $priezvisko); ?></li>
